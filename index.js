...
const { ensurePrsExist, readGHKey, checkGHKeyExists, getRepoName, isPrClosed } = require('./github');
...
async function getUnmergedBranches(sg, branches, baseBranch = DEFAULT_BASE_BRANCH) {
  return (await Promise.all(branches
    .map(b => {
      return isPrClosed({ sg, branch: b })
        .then(isClosed => ({
          branch: b,
          isClosed,
        }));
...
/**
 * @return {Promise.<{ context?: string, branches: Array.<BranchCfg> }>}
 */
async function getBranchesConfigInCurrentTrain(sg, config) {
  ...
  const key = Object.keys(trains).find(trainKey => {
    ...
    const branchNames = train.branches.map(b => getBranchName(b));
...
async function main() {
  ...
  const sortedTrainBranches = getBranchesInCurrentTrain(trainCfg.branches);
  const combinedTrainBranch = getCombinedBranch(trainCfg.branches);
  const trainBase = getTrainBase(trainCfg.branches);
  ...
  async function findAndPushBranches() {
    ...
    console.log("Pushing to branches:")
    branchesToPush.forEach((b, i) => {
      console.log(` -> ${b}`);
    })
  ...
  if (program.createPrs) {
    await findAndPushBranches();
    const prDict = await ensurePrsExist({
      sg,
